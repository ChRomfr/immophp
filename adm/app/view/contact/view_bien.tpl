...
function deleteContact(id){
	if( confirm('{$lang.Confirm_suppression_message} ?') ){
		window.location.href='{getLinkAdm("contact/delete/'+id+'?type=biens")}';
	}
}
//-->
</script>
...
<div id="bread">
	...
	<a href="{getLinkAdm("contact?type=biens")}" title="{$lang.Contact}">{$lang.Contact}</a>{$smarty.const.BREAD_SEP}
	#{$Contact.id}
</div>
<div class="showData">
...
<div class="tableau_simple">
    <table>
        ...
        <tr>
            <td>{$lang.Bien}</td>
            <td><a href="{getLink("annonce/detail/{$Contact.bien_id}")}" target="_blank">{$Contact.b_nom} - {$Contact.b_reference}</a></td>
        </tr>
    </table>
