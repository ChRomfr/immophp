<!-- adm/app/view/viewEditor/edit.tpl
...
<form class="well" method="post" action="{$Helper->getLinkAdm("viewEditor/edit/{$Vue->id}")}" id="formEditView">
	<fieldset>
		...
		<div class="control-group">
			<label>Code :</label>
			<div class="controls">
				<textarea rows="30" class="input-xxlarge" name="tpl[tpl]">{$Vue->tpl|htmlentities}</textarea>
			</div>
		</div>
		<div class="control-group">
			<label>Active :</label>
			<div class="controls">
				<select name="tpl[active]">
					<option value="0" {if $Vue->active == 0}selected=selected{/if}>{$lang.Non}</option>
					<option value="1" {if $Vue->active == 1}selected="selected"{/if}>{$lang.Oui}</option>
				</select>
			</div>
		</div>
		<div class="form-actions">
			<input type="hidden" name="tpl[id]" value="{$Vue->id}" />
